      ...
      <VCol v-for="role in rolesStore.roles" :key="role?.name" cols="12" sm="6" lg="4">
        <VCard>
          <VCardText class="d-flex align-center pb-1">
            ...
            <span v-if="roles.SUPER_ADMIN !== role?.name"> {{ role?.meta.moduleCount }} module(s) </span>&nbsp;
            <span v-if="roles.SUPER_ADMIN !== role?.name"> {{ role?.meta.totalPermissions }} permission(s)</span>
