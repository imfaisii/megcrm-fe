...
export const useBenefitTypesStore = defineStore('benefit-types', () => {
  const defaultModel = {
    id: null,
    name: null
  }

  const endPoint = '/benefit-types'
  const entity = 'Benefit Type'
  const selected = ref<BenefitType>(defaultModel)
  ...
  const fetchAll = async (options = {}) => {
    ...
    entries.value = data.benefit_types
