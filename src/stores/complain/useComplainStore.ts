...
export const useTeamStore = defineStore('complain', () => {
  const endPoint = '/complaints'
  const entity = 'Complain'
  const complains = ref<any>([]);
  ...
  const includes = ["createdBy", "complaintable", "comments"]
  ...
  const index = async (options = { include: includes.join(",") }) => {
    ...
    complains.value = data.complains
    meta.value = {
  ...
  const get = async (complainId: Number) => {
    isLoading.value = true
    selectedId.value = complainId
    const { data } = await useApiFetch(`${endPoint}/${complainId}`)
    selected.value = data.complain
    isLoading.value = false
  }
